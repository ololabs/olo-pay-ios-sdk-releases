...
class SettingsViewController : UIViewController, UITextFieldDelegate {
    ...
    var _displayLineItemsToggle = UISwitch()
    ...
    func setUITestingIdentifiers() {
        ...
        _displayLineItemsToggle.accessibilityIdentifier = UITestingIdentifiers.Settings.displayLineItemsToggle
    }
    ...
    func createScrollingStack(scrollView: UIScrollView) {
        ...
        if(_settingsType == .digitalWallet) {
            scrollStack.addArrangedSubview(createDigitalWalletLineItemsStack())
        }
    ...
    func createDigitalWalletLineItemsStack() -> UIStackView {
        let lineItemsStack = UIStackView()
        lineItemsStack.axis = .vertical
        lineItemsStack.distribution = .fill
        lineItemsStack.alignment = .fill
        lineItemsStack.spacing = 10

        lineItemsStack.addArrangedSubview(createSectionHeader(title: "Line Items Settings"))

        // Set up Display Line Items UI
        setupToggle(toggle: _displayLineItemsToggle, isOn: TestHarnessSettings.sharedInstance.displayLineItems, action: #selector(displayLineItemsToggled))
        let displayLineItemsRow = createHorizontalStack(leftView: createLabel(title: "Display Line Items"), rightView: _displayLineItemsToggle)
        lineItemsStack.addArrangedSubview(displayLineItemsRow)
        
        return lineItemsStack
    }
    ...
    @objc func displayLineItemsToggled() {
        TestHarnessSettings.sharedInstance.displayLineItems = _displayLineItemsToggle.isOn
    }
