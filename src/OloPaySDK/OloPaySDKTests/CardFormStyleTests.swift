...
class CardFormStyleTests: XCTestCase {
    ...
    func testConvertFrom_String_to_OloPayCardFormStyle() throws {
        let cases = [
            ("borderless", OPCardFormStyle.borderless), ("standard", OPCardFormStyle.standard),
            ("BORDERLESS", OPCardFormStyle.borderless), ("STANDARD", OPCardFormStyle.standard),
            ("randomString", OPCardFormStyle.standard), ("", OPCardFormStyle.standard)
        ]
        cases.forEach {
            XCTAssertEqual(OPCardFormStyle.convert(from: $0) == $1, true)
        }
    }
}
