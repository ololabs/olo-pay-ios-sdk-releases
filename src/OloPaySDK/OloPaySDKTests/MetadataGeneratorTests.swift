...
final class MetadataGeneratorTests: XCTestCase {
    ...
    func assertStaticMetadata(_ metadata: [String : String]) {
        ...
        let buildType = OPStorageWrapper.getPListValue(of: "SDK Build Type", from: "Info", as: String.self)?.lowercased() ?? ""
        XCTAssertEqual(buildType, metadata["BuildType"]?.lowercased())
