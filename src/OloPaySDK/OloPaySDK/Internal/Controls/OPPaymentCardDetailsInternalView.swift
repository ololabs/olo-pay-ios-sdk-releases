...
//Subclass of STPPaymentCardTextField to allow customization of CVV, Card, and Error images
class OPPaymentCardDetailsInternalView: STPPaymentCardTextField, STPPaymentCardTextFieldDelegate {
    ...
    // Closure property to change CVV image handler
    static var cvvImageClosure: OPCardBrandImageBlock {
        get { _cvvImageClosure }
        set { _cvvImageClosure = newValue }
    }
    ...
    // Default CVV image handler that uses Stripe's native images
    private static var _cvvImageClosure: OPCardBrandImageBlock =
    ...
    // Override to allow for cvv image customization
    @objc(cvcImageForCardBrand:) override class func cvcImage(for cardBrand: STPCardBrand) -> UIImage?
        { _cvvImageClosure(OPCardBrand.convert(from: cardBrand)) }
    ...
    @objc func paymentCardTextFieldDidBeginEditingCVC(_ textField: STPPaymentCardTextField) {
        cardDetailsDelegate?.paymentCardDetailsViewFieldDidBeginEditing(self, field: OPCardField.cvv)
    }

    @objc func paymentCardTextFieldDidEndEditingCVC(_ textField: STPPaymentCardTextField) {
        cardDetailsDelegate?.paymentCardDetailsViewFieldDidEndEditing(self, field: OPCardField.cvv)
    }
