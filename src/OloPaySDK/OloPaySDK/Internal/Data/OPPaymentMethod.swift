...
class OPPaymentMethod: NSObject, OPPaymentMethodProtocol {
    ...
    @objc required init(
        ...
        _fullName = _applePayConfig?.fullNameRequired ?? false ?
            pkPayment?.billingContact?.name?.fullNameOrNil() ??
            pkPayment?.shippingContact?.name?.fullNameOrNil() ??
            "" : ""
        
        _phoneticName = _applePayConfig?.fullPhoneticNameRequired ?? false ?
            pkPayment?.billingContact?.name?.phoneticRepresentation?.fullNameOrNil() ??
            pkPayment?.shippingContact?.name?.phoneticRepresentation?.fullNameOrNil() ??
            "" : ""
...
extension PersonNameComponents {
    func fullNameOrNil() -> String? {
        let given = givenName ?? ""
        let family = familyName ?? ""
        let components = [given, family].filter { !$0.isEmpty }
